...
int ShowMoStrContent(FILE *fpMo, unsigned int StrIndex, FILE *fpDst) {
    ...
    GetStrInfo(fpMo, &StrInfo, StrIndex);
    pOrigin = malloc(MO_STR_NUL_SIZE + StrInfo.LenOfOrigin * sizeof(char));
    fseek(fpMo, StrInfo.OffsetOfOrigin, SEEK_SET);
    fread(pOrigin, sizeof(char), MO_STR_NUL_SIZE + StrInfo.LenOfOrigin, fpMo);
    fprintf(fpDst, "%s\n", pOrigin);

    pTrans = malloc(MO_STR_NUL_SIZE + StrInfo.LenOfTrans * sizeof(char));
...
int Mo2Po(FILE *fpMo, FILE *fpDst) {
    ...
    for (i = 0; i < MoHeader.StringNumber; ++i) {
        ...
        pTemp = malloc(MO_STR_EXT_SIZE + MO_STR_NUL_SIZE + StrInfo.LenOfOrigin * sizeof(char));
        ...
        pTemp = malloc(MO_STR_EXT_SIZE + MO_STR_NUL_SIZE + StrInfo.LenOfTrans * sizeof(char));
